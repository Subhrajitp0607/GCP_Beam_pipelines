...
class DataflowOptions(PipelineOptions):
    ...
    def _add_argparse_args(cls, parser):
        parser.add_argument(
        '--input_subscription',default="projects/ethereal-aria-416604/subscriptions/gcp-yelp-stream-sub",
        help='Input PubSub subscription of the form "/subscriptions/<PROJECT>/<SUBSCRIPTION>".')
        parser.add_argument(
            '--output_table', type=str,default='ethereal-aria-416604:test.yelp_covid',
            help='Output BigQuery table for results specified as: PROJECT:DATASET.TABLE or DATASET.TABLE.')
        parser.add_argument(
            '--output_error_table', type=str,default='ethereal-aria-416604:test.error',
...
def run(argv=None):
    ...
    with beam.Pipeline(options=options) as pipeline:
        ...
        rows, error_rows = (
            pipeline
            | 'Read from Pub/Sub' >> beam.io.ReadFromPubSub(subscription=str(dataflow_options.input_subscription)).with_output_types(bytes)
